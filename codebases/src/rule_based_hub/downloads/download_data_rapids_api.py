...
# now = datetime.now()
# print("now ", type(now))
# print("current date", type(date.today()))
# print("current time ", type(now.strftime("%H:%M:%S")))

url = "https://alpha-vantage.p.rapidapi.com/query"

querystring = {"function":"FX_INTRADAY","interval":"15min","to_symbol":currency_to,"from_symbol":currency_from,"datatype":"json","outputsize":"compact"}
...
csv_filename = f"{currency_from.lower()}_{currency_to.lower()}.csv"
...
while True:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            result = data["Time Series FX (15min)"]
        ...
        if status == True and result != None:
            while (not has_file_access and interval > 1):
                ...
                # log the error in a logger file
                    ...
                    log_the_error(error) # helper function for log the error
                # Finally block
                finally:
                    interval -= 1
                    time.sleep(1.0)
                 
    # Catch block for writting the logger into a file system
    except Exception as error:
        log_the_error(error) # helper function for log the error
   
    #  Finally sleep for 5 mins to get the new data after every 5 mins 
    finally:
