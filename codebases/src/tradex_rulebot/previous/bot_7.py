...
def get_data_bars(symbols, rate, slow, fast):
    ...
    try:
        data = api.get_crypto_bars(symbols, rate, limit=20).df
        ...
        for signal in signals:
            if signals[signal] > 0:
                ...
                try:
                    ...
                    logging.warning('{} bought {}  {} shares, portfolio value {} 🤑💵'.format(datetime.datetime.now(tz).strftime("%x %X"), signal, signals[signal], api.get_account().equity))
                except:
