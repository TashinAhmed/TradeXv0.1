#!/usr/bin/env python
# this code can write after a pause.
import os, sys
import csv
import requests
...
# APIKEYID = "PK6T72QXFC3O0U5O5QJ0"
# APISECRETKEY = "NPDt8CYkgWUDomeMdzEAuSzO7Hi5DqsLB4ZTQf0Q"

APIKEYID = 'PK43VGN4DDS00MRVCWIH'
APISECRETKEY = 'EP6Hgol3yEh8WY5oUkaU0eY1kYEot9gByBBLACAB'
...
\__    ___/____    _____|  |__ |__| ____  
  ...
  |____|  (____  /____  >___|  /__|___|  /
    ...
    StockBot v{0}    +$ = :)  -$ = ;(\n
    https://github.com/tashinahmed/\033[0m\n\n""".format(STOCKBOT_VERSION)
                ...
                while True:
                    for stock in bought_stocks:
                        ...
                        if change_perc >= SELL_PERCENT_GAIN:
                            ...
                            print('placed sell order of stock {} ({}) for ${} (diff ${} {}%) (vol {})'.format(stock['symbol'], stock['company'], stock_price_sell, diff, change_perc, stock_vol_now))
                            sell_price = stock_price_sell * NUM_SHARES
                            stock_data = get_stock_info(stock['symbol'])
                            stock_vol_now = stock_data['chart']['result'][0]['indicators']['quote'][0]['volume'][0]
                            stock_data_csv.append([stock['symbol'], stock['company'], stock_price_buy, buy_time, stock_price_sell, sell_time, diff, change_perc, stock['volume'], stock_vol_now])
                    ...
                    while True:
                        for stock in bought_stocks:
                            ...
                            if (num_prices >= 15 and went_down > went_up) or (datetime.now(tz=TZ).hour == sell_eh \
                                ...
                                alpaca_order(stock['symbol'], side='sell')
                                stock_data = get_stock_info(stock['symbol'])
                                stock_data = stock_data['chart']['result'][1]['indicators']['loss'][1]
                                # stock_vol_now = stock_data['chart']['result'][0]['indicators']['quote'][0]['volume'][0]
